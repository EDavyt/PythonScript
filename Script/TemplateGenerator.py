...
def parse_list_field(value) -> List[str]:
    """Parse a field that may contain comma-separated values into a list."""
    if pd.isna(value):
        return []
...
def load_rules(rules_path: Path) -> List[dict]:
    ...
    if missing:
        raise ValueError(
            f"Missing expected columns in rules.xlsx: {', '.join(missing)}"
        )
...
def generate_document_json(
    ...
    base_name = template.stem
    out_name = f"{base_name}{form_title}_{document_guid_text}{template.suffix}".replace(
        " ", "_"
    )
    ensure_dir(destination_dir)
    out_path = destination_dir / out_name
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Template generator with specimen support."
    )
    parser.add_argument(
        "command",
